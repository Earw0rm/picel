...
in vec3 fragment_light_source_position;
...
void main(){
    ...
    vec3 light_direction = normalize(fragment_light_source_position - fragment_position_of_fragment);
    vec4 diffusion_component = vec4(mat.diffuse, 1) * max(dot(norm, light_direction), 0) * fragment_light_ambient_component;
    ...
    vec4 specular_component =  vec4(mat.specular, 1) * spec * fragment_light_ambient_component;


    vec4 ambient_component = vec4(mat.ambient, 1) * fragment_light_ambient_component; 
